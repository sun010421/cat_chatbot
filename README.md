...
- 사용자와 상호작용이 끝나고 5초가 지나면 테미는 자신의 위치를 확인하고 행정실 입구가 아니라면 **자동 복귀**한다.
...
> 앱 설정 창 (테미 설정 몇 개도 포함되어 있음)

<img src="https://user-images.githubusercontent.com/31657141/124870519-d2b92100-dffd-11eb-99ac-b04e06c711cd.PNG" width="500">

저장된 위치(대기 장소, 홈베이스, 선생님 자리)로 이동, 볼륨/조명 조절, 전원, 재시작, 위치 재조정, 설정 창 잠금 조정 등의 기능

('테미 설정' 을 눌러서 테미의 내재된 설정에 바로 접근 가능하기도 함)

** 볼륨, 전원 등의 Hard button 은 아무나 누르지 못하게 하기 위해 이렇게 설정을 통해서만 접근할 수 있게 되어 있다. 
  
** 간혹가다 테미가 지도 내의 자신의 위치를 헷갈려서 오작동을 하는 경우 - '위치 재조정'을 눌러주면 된다.

** 보안 상의 옵션으로 설정 창을 잠글 수 있으며, 비밀번호는 현재 301319 이다.

<br> 

** 앞에서 언급한 것과 같이 본 프로젝트 앱을 사용하기 위해서는 '키오스크 모드'로 설정을 해야 한다.

** 본 프로젝트 앱은 테미의 '기본 모드'를 중심으로 프로그래밍 되어 있다.

** 자동 복귀가 안되는 경우, 테미 설정에서 자동 복귀 기능이 켜져있는지, 앱이 설정을 다룰 권한이 주어졌는지 확인한다.

** 이외의 추가적인 설정은 Temi Center 에서 다룰 수 있다. (https://center.robotemi.com/)

   (Temi Center를 접근하기 위해서는 테미를 공장 초기화하고 새로 주인을 등록해야 함)

<br>

- 프로젝트의 다이얼로그플로우 연동은 네트워크 연결이 있는 경우에만 가능하다.

   (네트워크가 끊긴 경우 WIFI alert 활동이 시작됨)

- 연동을 위해서 구글 클라우드 프로젝트의 키가 필요하며, 이는 보안 상의 이유로 깃허브에 업로드되지 않는다.

- 테미의 상호작용 내용은 다이얼로그플로우에 기록되어, History 창에서 모두 확인 가능하도록 해놓았다.
...
- 테미는 매일 주중 아침 9시에 출근하고, 오후 5시에 퇴근한다. (Temi center 에서 원격으로 출퇴근 여부 확인 가능)
...
- 현재 테미는 신속한 이동을 위해 장애물을 회피하지 않고 기다리도록 설정되어 있다.

- 빠르고 정확한 이동을 위해 지도의 한 위치에서 다른 위치로 '경로'를 지정하는 것이 효율적이다.

   (한 위치를 설정하고, 그 자리에서 경로를 시작하고 이동시켜 끝 지점에서 경로를 끝내고 그 자리에서 위치도 설정)
...
- **얼굴인식**을 통한 사용자 반기기 기능 (상호작용이 끝나고 일정 시간이 지난 후에, 대기 장소에 있는 경우에만 시작됨)

- 상호작용이 끝나고 일정 시간이 지나면, 메인 화면으로 복귀하는 **Timeout** 기능

   (다음 사용자가 왔을 때 메인 화면에서 상호작용을 시작하게 해주기 위해서 추가한 기능)

<br>

- **와이파이 연결이 끊겼을 때 알리는 alert 활동** 기능

   (내부적으로는 다시 행정실 와이파이에 연결하려고 시도함)

<img src="https://user-images.githubusercontent.com/31657141/125023428-de6a1d80-e0b9-11eb-81e2-69016b15c3ff.png" width="500">

<br>

- **Configurable UI**

   (시기에 따라 학생들이 많이 하는 질문과 관련된 버튼을 메인 화면에 구성할 수 있게 함으로써 로봇의 유용함을 증진시킴)
